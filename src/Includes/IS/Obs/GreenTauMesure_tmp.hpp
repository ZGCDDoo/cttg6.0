...
class GreenTauMesure
{
  ...
  public:
                                                                                    ...
                                                                                    urngPtr_(urngPtr)
    {
        ...
        greenUpMCurrent_ = greenDownMCurrent_ = greenUpCurrent_;
    }

    SiteVector_t greenUpCurrent() const { return greenUpCurrent_; }
    SiteVector_t greenDownCurrent() const { return greenDownCurrent_; }

    SiteVector_t greenUpMCurrent() const { return greenUpMCurrent_; }
    SiteVector_t greenDownMCurrent() const { return greenDownMCurrent_; }
    ...
    void MeasureGreen(const double &tau)
    {
        ...
        SiteVector_t vec1M(KK);
        SiteVector_t vec2M(KK);
        ...
        for (size_t ii = 0; ii < ioModel_.indepSites().size(); ii++)
        {
            ...
            if (KK)
            {
                ...
                {
                    //tau  = tauRng - tauRng2
                    double tauRng = 0.0; //(*urngPtr_)() * dataCT_->beta_;
                    ...
                    for (size_t kk = 0; kk < KK; kk++)
                    {
                        ...
                        vec1M(kk) = dataCT_->green0CachedUp_(s1, rr, -tau + tauRng - tt);
                        vec2M(kk) = dataCT_->green0CachedUp_(rr, s2, tt - tauRng);
                    }

                    dotUp += LinAlg::Dot(vec1, *(dataCT_->MupPtr_), vec2);
                    dotDown += LinAlg::Dot(vec1, *(dataCT_->MdownPtr_), vec2);

                    dotUpM += LinAlg::Dot(vec1M, *(dataCT_->MupPtr_), vec2M);
                    dotDownM += LinAlg::Dot(vec1M, *(dataCT_->MdownPtr_), vec2M);
                }
            }

            const double green0 = dataCT_->green0CachedUp_(s1, s2, tau);
            const double green0M = dataCT_->green0CachedUp_(s1, s2, -tau);
            ...
            greenUpMCurrent_(ii) = sign * (green0M - dotUpM / static_cast<double>(N_T_INV));
            greenDownMCurrent_(ii) = sign * (green0M - dotDownM / static_cast<double>(N_T_INV));
        }
  ...
  private:
    ...
    SiteVector_t greenUpMCurrent_;   //for current config for -tau (Minus)
    SiteVector_t greenDownMCurrent_; //for current config for -tau (Minus)
};
